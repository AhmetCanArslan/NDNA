...
object Matcher {
    ...
    fun chips(f: Filters, name: String, description: String): List<String> {
        val haystack = "$name $description".lowercase()
        return (keywordChips(f, haystack) + filterChips(f, haystack)).distinct()
    }
    ...
    private fun filterChips(f: Filters, haystack: String): List<String> =
